...
def process_books_ingestion(contents_dir=DEFAULT_CONTENTS_DIR, output_dir=DEFAULT_OUTPUT_DIR,
    ...
    ensuring UTF-8 text encoding, and exports formatted JSONL batch files.
    """
    ...
    current_file = open(current_file_path, "w", encoding=encoding)
            ...
            try:
                ...
                for page_doc in pages:
                    ...
                    if isinstance(page_text, bytes):
                        page_text = page_text.decode(encoding, errors="replace")
                    clean_page_text = page_text.encode(encoding, errors="replace").decode(encoding)
